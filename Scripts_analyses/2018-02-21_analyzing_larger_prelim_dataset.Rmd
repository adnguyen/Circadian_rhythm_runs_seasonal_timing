...
ecl<-fread("../Data/2018-01-26_rhagoletis_masterdata_data_slice.csv")
str(ecl)
...
ddply(merg.dat,.(Host,organism,experiment),summarize,mean=mean(V1),sd=sd(V1))
```
...
mod<-aov(V1~organism*experiment,data=haw)
summary(mod)
ggplot(data=haw,aes(x=experiment,y=V1))+geom_boxplot()
...
mod1<-glm(eclosion_days~V1*experiment,family=gaussian,merg.dat)
summary(mod1)

v1.ave<-ddply(merg.dat,.(uniqueID,eclosion_days),summarize,mean=mean(V1))
ggplot(v1.ave,aes(x=mean,y=eclosion_days))+geom_point()+stat_smooth(method="lm")

merg.dat$biorhythm.time<-apply(merg.dat[,3:4],1,mean)
merg.dat2<-subset(merg.dat,biorhythm.time<50)
mod2<-glm(eclosion_days~biorhythm.time*experiment,family=gaussian,merg.dat2)
summary(mod2)

ggplot(merg.dat2,aes(x=biorhythm.time,y=eclosion_days,colour=organism))+geom_point()+stat_smooth()


mod3<-glm(biorhythm.time~organism,family=gaussian,merg.dat2)
summary(mod3)

#
ggplot(data=merg.dat,aes(x=V1,y=eclosion_days,colour=Host))+stat_smooth(se=FALSE)+geom_point(size=3)+facet_grid(experiment~.,scales="free")+ylim(0,100)+xlim(0,50)


#
flies<-subset(merg.dat,organism=="fly")

ggplot(data=flies,aes(x=V1,y=eclosion_days,colour=Host))+stat_smooth(se=FALSE)+geom_point(size=3)+facet_grid(experiment~.,scales="free")+ylim(0,100)+xlim(0,50)


###histogram of rep eclosion dates

ggplot(merg.dat,aes(x=eclosion_days,fill=Host))+geom_histogram(,alpha=.5)
ggplot(merg.dat,aes(x=eclosion_days,fill=organism))+geom_histogram(,alpha=.5)

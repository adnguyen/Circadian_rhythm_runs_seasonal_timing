...
dat$time2<-lubridate::hour(dat$time.conv)+lubridate::minute(dat$time.conv)/60+lubridate::second(dat$time.conv)/3600
head(dat$time2)
...
```{r}
dat.sum.counts<-ddply(dat,.(uniqueID,date,experiment,bins15),summarize,counts15=sum(counts)) # timing start 11:35 AM 
dim(dat.sum.counts)
str(dat.sum.counts)
...
whole.sa.dat<-ddply(dsct.sub,.(uniqueID,experiment),function(sub) sa.an(sub$counts15))
...
```{r}
#create time sequence for data set
dsct.sub.timeseq<-ddply(dsct.sub,.(uniqueID),transform,time=seq(1,length(uniqueID)))

ggplot(dsct.sub,aes(x=))
```

...
dat<-read.csv("../Data/2017-08-24_rhagoletis_data_sheet_2017-11-24_dataslice.csv")
str(dat)
```
...
head(cbind(dat3$resp_day11,dat3$resp1_sub,dat3$blank))
...
dat4<-subset(dat3,Site_name!="Blank" &  Resp_code == 1 & MSMR > 0 & organism!="wasp") # excluding blanks, wasps, msmr below 0, and experimental ones that don't have both day 11 and day 15
...
ggplot(dat4,aes(x=MSMR,y=eclosion_days,colour=Host))+geom_point(,size=5)+stat_smooth()+xlim(0,.01)+facet_grid(.~factor(Li.cor_1))+ylim(0,90)

ggplot(dat4,aes(x=MSMR,y=eclosion_days,colour=Host))+geom_point(,size=5)+stat_smooth()+xlim(0,.01)+ylim(0,90)
...
ggplot(dat4,aes(x=factor(Li.cor_1),y=MSMR))+geom_boxplot()
```

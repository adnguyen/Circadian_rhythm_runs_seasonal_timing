...
ggplot(all.data,aes(x=time_step,y=counts))+geom_line()+facet_grid(uniqueID~.)
...
#nall.data2<-ddply(nall.data,.(date,uniqueID),transform,time=seq(1,length(bins60),1))

#ddply(nall.data,.(date,uniqueID,experiment),summarize,time=length(seq(1,length(bins60),1)))
nall.data2<-na.omit(ddply(nall.data2,.(uniqueID,date),transform,order(as.Date(date,format="%Y/%m/%d"))))
...
ggplot(nall.data3,aes(x=time,y=counts))+geom_line()+facet_grid(uniqueID~.)#+geom_vline(xintercept=seq(0,900,24))#+geom_vline(xintercept=192)
...
dd<-data.frame(int=diff(IBL$mid_time)/4,time=IBL$mid_time[-35]/4/24)
...
plot(IBL$mid_time[-34]/4/24,diff(IBL$mid_time)/4,xlab="time (day)",ylab="interval in hr",las=1)
#lines(loess(diff(IBL$mid_time)~IBL$mid_time[-35],span=1))
s<-(IBL$mid_time[-34]/4/24)

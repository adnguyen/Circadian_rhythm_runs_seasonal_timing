...
raw.spec1<-spec.pgram(counts15$counts15,taper=0.1,kernel=c(4,4),spans=8,detrend=TRUE,log="no",demean = TRUE)
...
#1hr bin
raw.spec2<-spec.pgram(counts60$counts60,taper=0.1,,kernel=c(4,4,4),spans=12,detrend=TRUE,demean=TRUE)
...
dd<-data.frame(int=diff(IBL$mid_time)/4,time=IBL$mid_time[-10]/4/24,lighton=as.numeric(substr(dd$time,1,2))+6/24)
head(dd)
...
t<-data.frame(bout=diff(IBL$mid_time),mid=IBL$mid_time[-35])

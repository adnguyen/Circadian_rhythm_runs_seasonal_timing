...
counts15$time<-seq(0,805.75,.25)[-3224]
...
raw.spec1<-spec.pgram(counts15$counts15,taper=0,kernel=c(1,1,1),spans=3,detrend=TRUE)
...
wave.out0$period <- wave.out0$period/24
...
wave.out2$period <- wave.out2$period*10/24
...
wave.avg2[findpeaks(wave.avg2$power)[3,2],] #period in days 
...
```{r}
red<-redfit(counts15$counts15)

plot(red[["freq"]],red[["gxxc"]])
```
...
plot(IBL$mid_time[-288],diff(IBL$mid_time),xlab="time (hr)",ylab="interval in hr")
#lines(loess(diff(IBL$mid_time)~IBL$mid_time[-31],span=1))
lines(loess(diff(IBL$mid_time)~IBL$mid_time[-288],span=1))
...
#t<-data.frame(bout=diff(IBL$mid_time),mid=IBL$mid_time[-5])
t<-data.frame(bout=diff(IBL$mid_time),mid=IBL$mid_time[-288])
ggplot(data=t,aes(y=bout,x=mid))+stat_smooth()+geom_point()

...
x<-read.csv("2017-12-08_allm_all_monitorscombined.csv")
str(x)
library(lubridate)
t<-ymd_hms(paste(x$date,x$time,sep=""),tz="US/Eastern")
...
DJt_circ <- wavMRDSum(co1.3.1$count,levels=seq(1,7),keep.smooth=FALSE, keep.details=TRUE,reflect=TRUE,wavelet="s12",xform="modwt")
DJt_circ

#plot(co1.3.1$count,type="l",col="red")
#points(DJt_circ,col="purple")
plot(DJt_circ,col=rgb(0,0,.5,.5),type="l")
points(co1.3.1$count,type="l",col=rgb(.5,0,0,.5))

library(pracma)
IBL<-data.frame(findpeaks(DJt_circ,minpeakheight = 1))
...
par(mfrow=c(2,1))
par(mar=c(1,5,1,5))
plot(IBL$mid_time[-67],diff(IBL$mid_time),xlab="time (.1 hr)",ylab="interval in .1 hr",xlim=c(0,2000))
lines(loess(diff(IBL$mid_time)~IBL$mid_time[-67],span=1))
abline(v=seq(0,2400,240),lty="dotdash")


plot(co1.3.1$count,type="l")
abline(v=seq(0,2400,240),lty="dotdash")
...
plot(IBL$mid_time,IBL$Height,pch=20,col="black",type="l")
#plot(IBL$Height[-1],diff(IBL$mid_time),xlab="time (.1 hr)",ylab="interval in .1 hr")


hist(diff(IBL$mid_time),20)

###plotting specific ones 
detail3 <- wavMRDSum(co1.3.1$count,levels=3,keep.smooth=FALSE, keep.details=TRUE,reflect=TRUE,wavelet="s12",xform="modwt")
plot(detail3,type="l")
d3<-findpeaks(detail3)
plot(d3[-1,2],diff(d3[,2]),type="l")

detail7 <- wavMRDSum(co1.3.1$count,levels=7,keep.smooth=FALSE, keep.details=TRUE,reflect=TRUE,wavelet="s12",xform="modwt")
plot(detail7,type="l")
d7<-findpeaks(detail7)
plot(d7[-1,2],diff(d7[,2]),type="l")

...
#t1 - t32 = 6-37 
#write.csv(allm,"2017-12-17_allm_all_monitorscombined.csv")
#################################################
library(tidyr)
...
#write.csv(monitors.long,"2018-01-17_trik_dat_long.csv")
#################################################
...
ifelse(ymd_hms(head(monitors.long$fulltime),tz="US/Eastern") %within% test$int1,1,0)

output<-matrix(ncol=2,nrow=100)

for(i in head(monitors.long,100)){
  
  print(i[1])
  }


parse_fun<-function(fulltime=ymd_hms("2017-10-29 03:34:00",tz="US/Eastern"),monitor=1,position=8,data=test){
  #apply(test,1,function(x){ifelse(date %within%x$int1 & monitor==x$Trik_monitor & position==x$Trikinetics_position,"uniqueID","")})
  vec<-data.frame(x=ifelse(fulltime %within% test$int1 & monitor==test$Trik_monitor & position==test$Trikinetics_position,test$uniqueID,""))
  return(subset(vec,x !="<NA>" & x !=""))
}
parse_fun()


#ddply(head(monitors.long,100),.(time_step),transform,unique_row=if(fulltime %within% test$int1){return()})
ddply(head(monitors.long,100),.(time_step),transform,unique_row=fulltime)
ddply(head(monitors.long,100),.(time_step),transform,unique_row=ifelse(fulltime %within% test$int1 & ,row.names(test$int1),""))



apply(monitors.long[1:10,],1,FUN=parse_fun(fulldate=monitors.long$fulltime,monitor=monitors.long$monitor,position=monitors.long$position))


apply(monitors.long[1:10,],1,function(fulltime=ymd_hms(monitors.long$fulltime,tz="US/Eastern"),monitor=monitors.long$monitor,position=monitors.long$position,data=test){
  return(subset(data.frame(x=ifelse(fulltime %within% test$int1 & monitor==test$Trik_monitor & position==test$Trikinetics_position,test$uniqueID,"")),x !="<NA>" & x !=""))})


###install.packages("chron")
library(chron)

apply(monitors.long[1:10,],1,function(p){
  return(subset(data.frame(x=ifelse(p$monitor==test$Trik_monitor & position==p$position & p$fulltime < test$et2 & p$fulltime>test$et1,test$uniqueID,"")),x !="<NA>" & x !=""))})

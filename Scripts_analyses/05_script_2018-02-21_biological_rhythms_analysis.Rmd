...
dwf.fits<-par.dat%>%
  dplyr::group_by(uniqueID,experiment)%>%
  dplyr::summarise(circ=globdwf(vec=counts15,Jcirc=6),ult=globdwf(vec=counts15,Jcirc=4),int=globdwf(vec=counts15,Jcirc=5),up=globdwf(vec=counts15,Jcirc=7))
...
ggplot(data=merg.dat2,aes(y=ult,x=Host,colour=organism))+geom_boxplot()+facet_grid(treatment~experiment)

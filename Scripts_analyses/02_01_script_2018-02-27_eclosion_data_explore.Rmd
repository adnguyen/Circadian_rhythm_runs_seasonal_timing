...
treat.filt<-eclosions%>%
  filter(treatment!="" & treatment !="GC" & treatment !="F1")

#ggplot(treat.filt,aes(x=eclosion_days,fill=paste(Host,organism)))+geom_histogram(position="identity",alpha=.52)


ggplot(treat.filt,aes(x=eclosion_days,fill=paste(Host)))+geom_density(position="identity",alpha=.52)+facet_grid(treatment~.)

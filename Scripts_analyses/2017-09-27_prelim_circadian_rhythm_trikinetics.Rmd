...
d<-seq(time.x[1],time.x[2],length.out=50)
...
new.x<-ddply(x,.(cohort_day,tape),transform,seq(from=min(purge1,na.rm=TRUE),to=max(purge1,na.rm=TRUE),length.out=length(Ind_ID)))
